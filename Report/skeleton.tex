...
As discussed in sec. given a certain a neuron with preferred orientation $\phi_{i}$ and presented orientation $\theta_{c}$, we modelled the activity by von Mises equation (eq.\cite{vonmiseseq}), which leads to a bell shaped curve of activity as seen in fig.\cite{simpleMises}.
...
\plotfigureS{angles}{Two bars and the corresponding angles between them used in different formulas}{Parallel illusion}{.3}

\eql{h_{exp}(\phi_{i},\theta_{f},\varphi_{f},r_{f})=exp[-\cfrac{a}{r_{f}}(E(\phi_{i},\theta_{f},\varphi_{f})-E_{0})]}{mulh}

\eql{h_{add}(\phi_{i},\theta_{f},\varphi_{f},r_{f})=-\cfrac{a}{r_{f}}(E(\phi_{i},\theta_{f},\varphi_{f})-E_{0})}{addh}

\eql{\tau\cfrac{dr_{k}(t)}{dt}=-r_{k}(t)+[g(k)+\sum_{j}h(i)*r_{i}]^{+}}{addm}

\eql{\tau\cfrac{dr_{k}(t)}{dt}=-r_{k}(t)+g(k)*\prod_{j}h_mul_{j}(k)}{mulm}

\plottwofigures{misesNumberNeurons}{misesNumberNeurons2}{\bt{Visualization of sample input(output) to the model} (A) Random orientations. (B) All orientations from $0\degree$ to $180\degree$, for every $1.8\degree$.}{Sample input}

\plottwofigures{misesNumberA}{misesNumberB}{\bt{Visualization of sample input(output) to the model} (A) Random orientations. (B) All orientations from $0\degree$ to $180\degree$, for every $1.8\degree$.}{Sample input}
