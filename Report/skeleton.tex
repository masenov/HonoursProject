...
Our vision system is a complex machinery, able to transform the light change we detect with our eyes, to meaningful objects, perception of depth and motion. The information undergoes multiple stages of transformations by different parts of our visual cortex. The primary visual cortex (V1) is responsible for one of the first stages of vision processing. One of its functions is edge detection, which is determined by group of interacting neurons. Some visual illusions are believed to be caused in that specific part of V1. It is important to understand those illusions as they can give us an insight to how neural computation is done.

Here(in this report) we explore a model of the orientation selective neurons in the primary visual cortex. These(Those) neurons are responsible for detecting edges at different locations of what we see with our eyes. We have multiple neurons per location responding to different edge orientations. Their combined response forms our perception of the edge presented. Neurons for different locations are also connected to each other, influencing each other's responses. This can lead to change of the perceived orientations, leading to different visual illusions and effects. The connections between the neurons can be modeled using the elastica principle of smooth continuity - the smoother the curve connecting two neurons, the stronger they should be connected.
...
\plotfigure{gestaltgrouping}{Different groupings according to Gestalt psychology\cite{wilson2001encyclopedia}}{Gestalt}
...
\plotfigure{associationfield}{Association fields a) Solid lines represent stronger connections, doted lines - weaker b) Relation to smooth continuation \cite{keemink2015unified}}{Association fields}
...
\plotfiguresix{el1}{el2}{el3}{el4}{el5}{el6}{\bt{Elastica energies} Elastica energies for different bar configurations}{.3}{Elastica principle}
...
\eql{\tau\cfrac{dr}{dt}=-r+in+mod}{simplernn}
...
Different parts of the problem and work conducted are discussed in the report.
\begin{itemize}
...
\subsection{Tuning curves} \label{tuningcurves}
...
\subsection{Population coding} \label{populationcoding}
...
\plottwofigures{random_r}{random_r_torus}{\bt{Visualization of input and output of the model.} (a) Input of the model, a field of orientated bars. (B) Perceived bars, the more saturated the color, the stronger the response from the neurons for that particular bar. Some bars also may have different orientation from the input.}{Sample input}

As discussed in sec.\ref{twoorientations} given a certain a neuron with preferred orientation $\phi_{i}$ and presented orientation $\theta_{c}$, we modelled the activity by von Mises equation (eq.\ref{vonmiseseq}), which leads to a bell shaped curve of activity as seen in fig.\ref{simpleMises}.

\eql{g(\phi_{i},\theta_{c})=A_{c}exp(k_{c}cos2[\phi_{i}-\theta_{c}])}{vonmiseseq}


\plotfigureS{simpleMises}{\bt{Tuning curve.} Activity of a neuron with a preferred orientation 90° for different presented orientations}{Parallel illusion}{.5}
...
\plottwofigures{misesNumberNeurons}{misesNumberNeurons2}{\bt{Effect on the population activity, for different number of orientation selective neurons} (a) Presented orientation is $90\degree$. (b) Presented orientation is $45\degree$ In our experiments we are going to use 32 orientation selective neurons per bar of the input.}{Sample input}
...
\plottwofigures{misesNumberA}{misesNumberB}{\bt{Effect of $A_{c}$ and $k_{c}$ on von Mises function} (a) $A_{c}$ controls the strength of the response. (b) $k_{c}$ controls the variance.}{Sample input}



So far we described the activity of a neuron, assuming no modulation from nearby neurons. We will use the setup in fig.\ref{angles} to describe how we calculate the elastica modulation between different neurons and finally how put everything together in a final model.

\plotfigureS{angles}{\bt{Two bars} Two bars and the corresponding angles between them used in different formulas. We will refer to the orange bar as center and the blue line as flanker. \cite{keemink2015unified}}{Parallel illusion}{.3}
...
\eql{E(\theta_{c},\theta_{f},\phi_{f})\approx4(\beta_{c}^{2}+\beta_{f}^{2}-\beta_{c}\beta_{f})}{elasticaapproximation}
...
\eql{h_{add}(\phi_{i},\theta_{f},\varphi_{f},r_{f})=-\cfrac{a}{r_{f}}(E(\phi_{i},\theta_{f},\varphi_{f})-E_{0})}{addh}
...
Finally, eq.\ref{addm} shows the full equation of updating the response rate of a neuron.

\eql{\tau\cfrac{dr_{i}(t)}{dt}=-r_{i}(t)+[g_{i}+\sum_{j}h_{add}(\phi_{i},\theta_{f}^{j},\varphi_{f}^{j},r_{f}^{j})r_{j}(t-1)]^{+}}{addm}
...
\plotfourfigures{explainEl1}{explainEl2}{explainEl3}{explainEl4}{\bt{Elastica principle, intuition for static and dynamic model} (a) Strength of the connection between a stack of neurons and nearby neuron tuned for horizontal position (the more saturated the color the stronger the connection) (b) Strength of the spiking of the stack when a bar is presented on the left (the thicker the bar, the stronger the spike) (c) Combined a and b (d) We have a stack of neurons for every bar in our model.}{1}
...
Subfigures (a), (b) and (c) represent $h_{add}$, $r_{j}(t-1)$ and $h_{add}r_{j}(t-1)$ respectively in eq.\ref{addh}.
...
\begin{figure}[H] \label{tiltexperiments}
\caption{}
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.5\linewidth]{fig5_B2}
  \caption{}
\end{subfigure}%
\hfill
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.5\linewidth]{fig5_C2}
  \caption{}
\end{subfigure}
\hfill
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.5\linewidth]{fig5_D2}
  \caption{}
\end{subfigure}%
\hfill
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.5\linewidth]{fig5_F2}
  \caption{}
\end{subfigure}
\floatfoot{Various tilt experiments. a) Vary the orientations of two horizontal flankers. b) Rotate the two flankers around the center bar. c) Vary the orientation of two vertical flankers d) Vary the orientation of six flanker arranged in a hexagon shape around a center bar}
\end{figure}
...
\plotfigureS{fig5_B2}{Tilt Effect 1}{Tilt Effect 1}{0.3}
...
\plotfigureS{matrix_b}{\bt{Weight matrix}}{Weight Matrix 1}{0.5}
...
\plottwofigures{matrix_b_nomod}{matrix_b_torus}{\bt{Variations of the weight matrix} (a) No recurrent connections back to the flankers (b) Assuming the three bars line on a circle rather than on a line, then the distance between any of them is the same}{1}
...
\plotthreefigures{p_cen_b}{d_cen_b}{d_flan_b}{\bt{Experienced tilt illusions for different flanker orientations} (a) Tilt illusion of the center bar. The experiment is reproduced based on \cite{keemink2015unified}. (b) Tilt illusion of the center bar, in the dynamic model. (c) Tilt illusion of the flanker bars, in a dynamic model.}{Experienced tilt illusions}{1}
...
It is useful to also explore the dynamics of the system, how the response of the neurons vary with respect to time. In fig.\ref{responseB} we show the change of the orientation selective neurons for center bar, magnitude of the overall activity and change in orientation with respect to time. We notice the gradual change of the magnitude of the neurons' responses, as well as the gradual shift in their response, based on nearby modulation. 

\plotthreefigures{responseB}{tilt_time_b}{mag_time_b}{\bt{Change in perceived orientation and magnitude of the response, with respect to time.} (For this particular experiment both flankers have orientation $135\degree$, $r_{f}=6$) (a) Population coding of the center bar for different time steps. (b) Change in perceived orientation. (c) Change in the magnitude of the response.}{Experienced tilt illusions}{1}

(more experiments for with the different weight matrices, values of $a$

\subsection{Tilt effect 2}
\plotfigureS{fig5_C2}{Tilt Effect 2}{Tilt Effect 2}{0.3}
...
\plotfigureS{fig5_D2}{Tilt Effect 3}{Tilt Effect 3}{0.3}
...
\plotfigureS{fig5_F2}{Tilt Effect 4}{Tilt Effect 4}{0.3}
\plottwofigures{p_cen_f}{d_cen_f}{Experienced tilt illusions for different modulation constants. (a) Tilt illusion of the center bar for different flaker orientations. The experiment reproduced is from \cite{keemink2015unified}, fig.5, F (b) Tilt illusion of the center bar for different flanker orientations, in the dynamic mode. (c) Tilt illusion of the flanker bars for different flanker orientations, in a dynamic model}{Experienced tilt illusions}{1}

\plotfiguresix{d_flan_f1}{d_flan_f2}{d_flan_f3}{d_flan_f4}{d_flan_f5}{d_flan_f6}{\bt{Additive and Multiplicative model} (a) Elastica energy (b) h, additive (c) h, multiplicative (d) Central drive (e) Final response for additive model (f) Final response for multiplicative model}{.49}{Additive and Multiplicative mode}

\plottwofigures{p_cen_f3}{d_cen_f3}{Experienced tilt illusions for different modulation constants. (a) Tilt illusion of the center bar for different flaker orientations. The experiment reproduced is from \cite{keemink2015unified}, fig.5, F (b) Tilt illusion of the center bar for different flanker orientations, in the dynamic mode. (c) Tilt illusion of the flanker bars for different flanker orientations, in a dynamic model}{Experienced tilt illusions}{1}

\plotfiguresix{d_flan_f13}{d_flan_f23}{d_flan_f33}{d_flan_f43}{d_flan_f53}{d_flan_f63}{\bt{Additive and Multiplicative model} (a) Elastica energy (b) h, additive (c) h, multiplicative (d) Central drive (e) Final response for additive model (f) Final response for multiplicative model}{.49}{Additive and Multiplicative mode}

\plottwofigures{p_cen_f6}{d_cen_f6}{Experienced tilt illusions for different modulation constants. (a) Tilt illusion of the center bar for different flaker orientations. The experiment reproduced is from \cite{keemink2015unified}, fig.5, F (b) Tilt illusion of the center bar for different flanker orientations, in the dynamic mode. (c) Tilt illusion of the flanker bars for different flanker orientations, in a dynamic model}{Experienced tilt illusions}{1}

\plotfiguresix{d_flan_f16}{d_flan_f26}{d_flan_f36}{d_flan_f46}{d_flan_f56}{d_flan_f66}{\bt{Additive and Multiplicative model} (a) Elastica energy (b) h, additive (c) h, multiplicative (d) Central drive (e) Final response for additive model (f) Final response for multiplicative model}{.49}{Additive and Multiplicative mode}



\section{Pop out effects and contour extraction}
...
\eql{h_{mul}(\phi_{i},\theta_{f},\varphi_{f},r_{f})=exp[-\cfrac{a}{r_{f}}(E(\phi_{i},\theta_{f},\varphi_{f})-E_{0})]}{mulh}

\eql{\tau\cfrac{dr_{i}(t)}{dt}=-r_{i}(t)+g_{i}\prod_{j}h_{mul}(\phi_{i},\theta_{f}^{j},\varphi_{f}^{j},r_{f}^{j})r_{j}(t-1)}{mulm}
...
\plotfiguresix{el_ener}{h_add}{h_mul}{cen}{ad_comb}{mul_comb}{\bt{Additive and Multiplicative model} (center bar orientation is $\cfrac{\pi}{4}$, flanker bar orientation is $\cfrac{\pi}{2}$) (a) Elastica energy (b) h, additive (c) h, multiplicative (d) Central drive (e) Final response for additive model (f) Final response for multiplicative model}{.49}{Additive and Multiplicative mode}
...
\plotfigure{ADD_vs_MULT}{\bt{Additive vs Multiplicative rules for different flanker orientations.} (center bar orientation is $\cfrac{\pi}{4}$) Although the tilt effects happen at different orientations and the strength of modulation is different, the change in the effect as we vary the flanker orientation is similar.}{AdMp}
...
\addmilestone{at=phase-1.230,direction=230:1.7cm,text={Neural Computation},text options={below}}
